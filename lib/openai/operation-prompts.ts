...
3. 各ステップで以下を返す:
   ...
   - x, y, width, height: 画像座標の 0〜1000 バウンディングボックス (整数推奨)
     ...
     * **width と height は矩形の「幅」と「高さ」です。右下の座標 (x2, y2) ではありません**
       - 正しい例: 中央の200×50ボタン → { x: 400, y: 475, width: 200, height: 50 }
       - 間違い例: 中央の200×50ボタンを { x: 400, y: 475, width: 600, height: 525 } と返すのは禁止 (これは x2, y2 形式)
     * 対象要素の外周にぴったり合わせてください。周囲の空白を大きく含めないでください (tight bbox)
     * テキストリンクや細い行 (GitHub のファイル一覧など) の場合、そのリンク文字列または行のみを囲んでください。典型的な高さは 20〜60 程度で、数百にはなりません
     * リストの特定の行を指す場合、その行の y 位置をしっかり合わせてください (上下の別の行に重ならない)
   ...
   - confidence: 0〜1 の信頼度。位置に 10% 以上の不確実さがあれば 0.5 未満にすること
...
- 位置が 10% 以上曖昧なら steps に入れず、unresolved に reason/question を書いて回してください
- クリックの自動実行は行わない。案内のみ
- bbox は必ず 0〜1000 のスケール (正規化された 0〜1 では返さない)
- bbox の width と height は「幅」「高さ」であって (x2, y2) ではない
- bbox は対象要素にぴったり合わせ、余白を大きく含めない
