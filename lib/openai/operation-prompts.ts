...
3. 各ステップで以下を返す:
   ...
   - x, y, width, height: 画像座標の 0〜1000 正規化バウンディングボックス (画像の左端が 0, 右端が 1000)
   - targetType: button / menu / tab / link / file / icon / unknown のいずれか
   - titleJa: 対象要素の短い日本語タイトル (例: "設定アイコン")
   - actionJa: 何をするかの日本語の命令形 (例: "右上の歯車アイコンをクリック")
   - detailJa: 補足説明 (例: "クリックすると設定画面が開きます")
   - confidence: 0〜1 の信頼度
